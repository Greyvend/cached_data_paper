...
\author{Mosin~S.~V., Zykin~S.~V.\footnote{Supported by the Russian Foundation for Basic Research under
grant \#12-07-00066-a.}}
\authorcr{Mosin~S.~V., Zykin~S.~V.}
\keywords{Relational databases, Caching, Truth space}
\rutitle{Кэширование запросов к реляционной базе данных с использованием областей истинности}
\ruauthor{Мосин С.В., Зыкин С.В.}

\rukeywords{реляционная база данных, кэш, область истинности}
...
\ruaffiliation{\small Омский филиал Федерального государственного бюджетного учреждения науки Института математики им. С.Л.~Соболева Сибирского отделения Российской академии наук \\644043, Россия, г. Омск,
ул. Певцова, 13,}
%%%%
\email{svmosin@gmail.com, szykin@mail.ru}
\authorinform{
\textbf{Мосин Сергей Владимирович},\\Омский филиал Федерального государственного бюджетного учреждения науки Института математики им. С.Л.~Соболева Сибирского отделения Российской академии наук, аспирант\\
\textbf{Зыкин Сергей Владимирович},\\Омский филиал Федерального государственного бюджетного учреждения науки Института математики им. С.Л.~Соболева Сибирского отделения Российской академии наук, д-р техн. наук, профессор
}



\ruabstract{
В данной статье предлагается новый метод кэширования запросов к реляционной базе данных
для систем с центральным сервером и распределенными клиентами. Данные загружаются в
клиентский кэш, основываясь на запросах, выполненных на сервере БД. Эти запросы имеют специальный вид, называемый "универсальный реляционный запрос". Большинство запросов может быть записано в этом виде. Кроме того, эта форма записи позволяет анализировать корректность запроса, проверяя свойство соединения без потери информации (СБПИ). Последовательные запросы могут исполняться на клиенте, используя кэш, если удастся определить, что результаты искомого запроса полностью содержатся в кэше. Для осуществления такой проверки анализируются области истинности логических ограничений искомого запроса и запросов, результаты которых уже содержатся в кэше. Этот метод может быть использован для определения недостающих в кэше данных и последующего запроса только на эти данные. Проблема актуализации данных не затрагивается в этой статье. Однако она может быть решена путем учета запросов на сервере и обновлении данных при помощи триггеров.}
...
based on queries executed on the server. These
queries have a special form called "universal relational query". The majority of
