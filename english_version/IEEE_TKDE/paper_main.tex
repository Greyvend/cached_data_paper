...
the Siberian Branch of the Russian Academy of Sciences, Omsk, Russia.\protect\\
...
data representations logically described as multidimensional tables. We consider
queries in a form of relational algebra expressions, similar to "universal
relational query". Logical formulas in the queries are
...
representations. Two basic methods are used: statical \cite{baralis,
gupta, gupta-mumick} and dynamical \cite{scheuermann, shim, kalnis, chang}.
...
of query execution. Data Warehouses or Relational Databases in
the hierarchical form are used as a data source. Special queries for data manipulation are
considered. In our paper, we use split dimension generation for the data
...
In the \cite{Keller96} paper, the authors analyze the cache content,
...
Listed variants of operations don't use every single SQL capability. For
example, we don't use the $\n{EXISTS}$ predicate, because it has no expanded attribute
...
$NULL$ value on a row $t$, we then get $\n{UNKNOWN}$ value for the whole
...
After the modification it will be the following: 
...
can also note that if rows don't have undefined values, the initial formula $F$
...
transformation on $t$ row will be $\n{TRUE}$, which is, indeed, not so obvious.
...
software's ability to compute Truth spaces of formulas, as it is shown in example
1.
\end{note}
...
The conditions given in the aforementioned theorem guarantee the data to build $P^{\ast}$ to be
...
It is possible to use several IRs to build a resulting representation. First,
...
Just like in the previous case, there are some additional conditions that allow us
...
dimensions that form a hypercube changes rarely, so we can create new dimensions
...
the MOLAP approach, the technical support staff is responsible for periodical
...
only modified data decreases the refresh time period. However, it should be
noted again that those dimensions don't change often.
...
increases the overall time elapsed. But the constant GPU memory growth and
comparatively small data amount required for dimensions building make the usage
...
The authors would like to thank the Russian Foundation for Basic Research for the
grant \#12-07-00066-a.
